...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    lazy val xmap = x.toMap
    lazy val ymap = y.toMap withDefaultValue 0
    (xmap foldLeft List[(Char, Int)]()){
      case (acc, (char, count)) =>
        val newCount: Int = count - ymap(char)
        if (newCount > 0) (char, newCount) :: acc
        else acc
    }.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
          ...
          sent <- sentenceAnagrams(subtract(occurrences, combo))
        } yield word :: sent
    }
  ...
  def sentenceAnagramsMemo(sentence: Sentence): List[Sentence] = {
    val gramMap = scala.collection.mutable.HashMap[Occurrences, List[Sentence]]() withDefaultValue List()
